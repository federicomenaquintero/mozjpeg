                      ...
                      int num_rows)
{
  ...
  if (simd_support & JSIMD_AVX2)
    ...
    mmxfct(input->output_width, input_buf, input_row, output_buf, num_rows);
}
                      ...
                      JSAMPARRAY input_data, JSAMPARRAY output_data)
{
  if (simd_support & JSIMD_AVX2)
    ...
    jsimd_h2v2_downsample_mmx(image_width, args->max_v_samp_factor,
                      ...
                      JSAMPARRAY input_data, JSAMPARRAY output_data)
{
  if (simd_support & JSIMD_AVX2)
    ...
    jsimd_h2v1_downsample_mmx(image_width, args->max_v_samp_factor,
                              comp_args->v_samp_factor, comp_args->width_in_blocks,
                              input_data, output_data);
}
                           ...
                           JDIMENSION in_row_group_ctr, JSAMPARRAY output_buf)
{
  ...
  if (simd_support & JSIMD_AVX2)
    avx2fct(output_width, input_buf, in_row_group_ctr, output_buf);
  else if (simd_support & JSIMD_SSE2)
    sse2fct(output_width, input_buf, in_row_group_ctr, output_buf);
  else
    mmxfct(output_width, input_buf, in_row_group_ctr, output_buf);
}
...
{
  if ((simd_support & JSIMD_SSE2) && IS_ALIGNED_SSE(jconst_idct_red_sse2))
    jsimd_idct_2x2_sse2(comp_args->dct_table, coef_block, output_buf,
                        output_col);
  else
    jsimd_idct_2x2_mmx(comp_args->dct_table, coef_block, output_buf, output_col);
}
...
{
  if ((simd_support & JSIMD_SSE2) && IS_ALIGNED_SSE(jconst_idct_red_sse2))
    jsimd_idct_4x4_sse2(comp_args->dct_table, coef_block, output_buf,
                        output_col);
  else
    jsimd_idct_4x4_mmx(comp_args->dct_table, coef_block, output_buf, output_col);
}
                 ...
                 JDIMENSION output_col)
{
  if (simd_support & JSIMD_AVX2)
    ...
    jsimd_idct_islow_mmx(comp_args->dct_table, coef_block, output_buf,
                         output_col);
}
                 ...
                 JDIMENSION output_col)
{
  if ((simd_support & JSIMD_SSE2) && IS_ALIGNED_SSE(jconst_idct_ifast_sse2))
    ...
    jsimd_idct_ifast_mmx(comp_args->dct_table, coef_block, output_buf,
                         output_col);
}
                 ...
                 JDIMENSION output_col)
{
  if ((simd_support & JSIMD_SSE2) && IS_ALIGNED_SSE(jconst_idct_float_sse2))
    ...
    jsimd_idct_float_sse(comp_args->dct_table, coef_block, output_buf,
                         output_col);
  else
    jsimd_idct_float_3dnow(comp_args->dct_table, coef_block, output_buf,
                           output_col);
}
